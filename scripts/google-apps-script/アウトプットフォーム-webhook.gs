/**
 * アウトプットフォーム（Googleフォーム）送信時にCRM Webhook + Slack通知を行うApps Script
 *
 * === セットアップ手順 ===
 * 1. 「アウトプットフォーム」のGoogleフォームを開く
 * 2. 右上の「︙」→「スクリプトエディタ」
 * 3. このコードを貼り付けて保存
...
const SLACK_CHANNEL = "C094KPN4JCA";
const FORM_NAME = "アウトプットフォーム";
const BOT_USERNAME = "アウトプットくん";
...
function onFormSubmit(e) {
  try {
    ...
    // 2. Slack へ直接通知
    const message = "名前：" + (data["名前"] || "") + "\nLINE登録名：" + (data["LINE登録名"] || "") + "\n閲覧済教材：" + (data["閲覧済教材"] || "") + "\n新たに学んだこと：" + (data["新たに学んだこと"] || "") + "\nわからない・質問したいこと：" + (data["わからない・質問したいこと"] || "") + "";
