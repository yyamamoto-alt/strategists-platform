/**
 * メンター採用（27卒）（Googleフォーム）送信時にCRM Webhook + Slack通知を行うApps Script
 *
 * === セットアップ手順 ===
 * 1. 「メンター採用（27卒）」のGoogleフォームを開く
 * 2. 右上の「︙」→「スクリプトエディタ」
 * 3. このコードを貼り付けて保存
...
const SLACK_CHANNEL = "C0A3K7WVBQA";
const FORM_NAME = "メンター採用（27卒）";
const BOT_USERNAME = "採用bot";
...
function onFormSubmit(e) {
  try {
    ...
    // 2. Slack へ直接通知
    const message = "名前：" + (data["名前"] || "") + "\nLine名：" + (data["Line名"] || "") + "\nメールアドレス：" + (data["メールアドレス"] || "") + "\n学歴・学部：" + (data["学歴・学部"] || "") + "\n内定先：" + (data["内定先"] || "") + "\n所属選抜コミュニティ：" + (data["所属選抜コミュニティ(なしならなしと回答)"] || "") + "\n対策開始時期：" + (data["対策開始時期"] || "") + "\n壁打ち回数：" + (data["壁打ち回数"] || "") + "\n利用有料サービス：" + (data["利用有料サービス(あれば)"] || "") + "\n稼働希望時間／週：" + (data["稼働希望時間／週"] || "") + "\nその他連絡事項：" + (data["その他連絡事項"] || "") + "\nビジネス経験：" + (data["ビジネス経験(起業・インターンシップ等)"] || "") + "\nリーダーシップ経験：" + (data["リーダーシップ経験(学生団体・部活・サークル・起業・インターン等)"] || "") + "";
