    ...
    <div className="p-6 bg-surface min-h-screen">
      ...
      <div className="space-y-3">
        {mockApplications.map((app) => {
          ...
          return (
            <div key={app.id} className="bg-surface-elevated rounded-xl overflow-hidden">
              <button onClick={() => setExpandedId(isExpanded ? null : app.id)} className="w-full p-4 text-left flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-brand rounded-full flex items-center justify-center text-sm font-bold text-white">{app.name[0]}</div>
                  <div><p className="text-sm text-white font-medium">{app.name}</p><p className="text-xs text-gray-400">{app.email}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${statusBadges[app.status] || "bg-white/10 text-gray-300"}`}>{statusLabels[app.status] || app.status}</span>
