...
interface Props {
  courses: Course[];
  courseDataMap: Record<string, CourseData>;
  forms: FormItem[];
...
function statusBadge(s: string) {
  switch (s) {
    case "完了": return <span className="absolute top-2 right-2 bg-green-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-medium">完了</span>;
    case "閲覧済み": return <span className="absolute top-2 right-2 bg-blue-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-medium">閲覧済</span>;
    case "進行中": return <span className="absolute top-2 right-2 bg-yellow-500 text-black text-[10px] px-1.5 py-0.5 rounded-full font-medium">進行中</span>;
    default: return null;
  }
}
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden">
        {hasThumbnail ? (
          <img src={(lesson as any).thumbnail_url} alt={lesson.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-3">
        <p className="text-sm text-gray-200 group-hover:text-white font-medium line-clamp-2 leading-snug">{lesson.title}</p>
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-[10px] text-gray-500">{lesson.lesson_type}</span>
          {lesson.duration_minutes && <span className="text-[10px] text-gray-600">{lesson.duration_minutes}分</span>}
        </div>
      </div>
    </Link>
  );
...
export function CoursePageClient({ courses, courseDataMap, forms, noAccessMessage }: Props) {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-400 text-sm">{noAccessMessage || "コースがありません"}</p>
        </div>
      </div>
    );
  }
  ...
  const data = courseDataMap[course.id] || { modules: [], progressMap: {} };
  const { modules, progressMap } = data;
  ...
  const handleCourseSwitch = (courseId: string) => {
    ...
    const newData = courseDataMap[courseId];
    if (newData) setExpandedModules(new Set(newData.modules.map((m) => m.id)));
  };
    ...
    <div className="p-6 bg-surface min-h-screen">
          ...
          {courses.map((c) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                c.id === course.id
                  ? "bg-brand text-white"
                  : "bg-surface-elevated text-gray-400 hover:text-white hover:bg-white/[0.06] border border-white/10"
              }`}
            >
              {c.title}
            </button>
      ...
      <div className="bg-gradient-to-r from-[#0a0a0a] to-brand rounded-xl p-8 mb-8">
        ...
        <div className="flex items-center gap-6 text-sm text-white/70 mb-4">
          ...
          {completedLessons > 0 && <span className="flex items-center gap-1"><CheckCircle className="w-4 h-4" />{completedLessons}/{totalLessons} 完了</span>}
        </div>
        {totalLessons > 0 && (
                          ...
                          {hasThumbnails ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                              {lessons.map((lesson, i) => (
                                <LessonCard key={lesson.id} lesson={lesson} slug={slug} status={progressMap[lesson.id] || ""} index={i} />
                              ))}
                            </div>
                          ) : (
          ...
          {forms.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {forms.map((form) => (
                  <a key={form.id} href={form.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-surface-elevated rounded-lg px-5 py-3.5 hover:bg-white/[0.06] transition-colors group">
