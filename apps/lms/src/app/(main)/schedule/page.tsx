      ...
      {mockEnrollments.length === 0 ? (
        ...
        <div className="space-y-4">
          {mockEnrollments.map((enrollment) => {
            ...
            return (
              <div key={enrollment.id} className="bg-gray-800 rounded-xl p-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="bg-gray-900 rounded-lg p-3"><p className="text-xs text-gray-500 mb-1">受講開始</p><p className="text-sm text-white">{new Date(enrollment.enrolled_at).toLocaleDateString("ja-JP")}</p></div>
                  <div className="bg-gray-900 rounded-lg p-3"><p className="text-xs text-gray-500 mb-1">期限</p><p className="text-sm text-white">{enrollment.deadline ? new Date(enrollment.deadline).toLocaleDateString("ja-JP") : "未設定"}</p></div>
                  <div className="bg-gray-900 rounded-lg p-3"><p className="text-xs text-gray-500 mb-1">残り日数</p><div className="flex items-center gap-1">{daysLeft !== null && daysLeft <= 7 && <AlertTriangle className="w-3 h-3 text-red-400" />}<p className={`text-sm font-semibold ${daysLeft !== null && daysLeft <= 7 ? "text-red-400" : "text-white"}`}>{daysLeft !== null ? `${daysLeft}日` : "-"}</p></div></div>
                </div>
              </div>
            );
          })}
