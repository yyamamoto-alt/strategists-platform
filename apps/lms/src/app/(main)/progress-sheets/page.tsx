...
function RatingBar({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 bg-gray-700 rounded-full h-2"><div className="bg-primary-500 h-2 rounded-full transition-all" style={{ width: `${(value / 5) * 100}%` }} /></div>
    ...
    <div className="p-6 bg-gray-950 min-h-screen">
      ...
      <div className="space-y-3">
        {mockProgressSheets.map((sheet) => {
          ...
          return (
            <div key={sheet.id} className="bg-gray-800 rounded-xl overflow-hidden">
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-gray-700 pt-4 space-y-3">
                  ...
                  {sheet.next_goals && <div><h4 className="text-xs font-semibold text-gray-400 mb-1">次回の目標</h4><p className="text-sm text-blue-300">{sheet.next_goals}</p></div>}
                </div>
              )}
            </div>
          );
