    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm border p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(cumulative)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border p-4">
          <p className="text-xs text-gray-500">平均LTV</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(avgLTV)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border p-4">
          <p className="text-xs text-gray-500">スクール平均単価</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(avgSchoolPrice)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border p-4">
          <p className="text-xs text-gray-500">人材紹介併用率</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatPercent(agentRate)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold mb-4">月別売上推移（セグメント別）</h2>
          <RevenueChart data={mockRevenueMetrics} />
        </div>
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h2 className="text-lg font-semibold mb-4">ファネル推移（申込→成約）</h2>
      ...
      {/* 月別P/Lテーブル */}
      <div className="bg-white rounded-xl shadow-sm border p-6">
        <h2 className="text-lg font-semibold mb-4">月別P/L概要</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {mockRevenueMetrics.map((r, i) => {
                ...
                return (
                  <tr key={r.period} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-2 px-3 font-medium">{r.period}</td>
                    <td className="py-2 px-3 text-right font-medium">{formatCurrency(r.confirmed_revenue)}</td>
                    <td className="py-2 px-3 text-right text-gray-500">{formatCurrency(r.projected_revenue)}</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(r.school_revenue)}</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(r.agent_revenue)}</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(r.content_revenue)}</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(r.other_revenue)}</td>
                    <td className="py-2 px-3 text-right">{f?.applications || "-"}</td>
                    <td className="py-2 px-3 text-right">{f?.closed || "-"}</td>
                    <td className="py-2 px-3 text-right">{f ? formatPercent(f.closing_rate) : "-"}</td>
                  </tr>
                );
              })}
            </tbody>
