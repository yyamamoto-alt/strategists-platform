...
const CATEGORY_PROMPTS: Record<string, string> = {
  marketing: `あなたはマーケティング戦略の専門家です。以下のデータを分析し、マーケティングに関する経営示唆を3つ、日本語で提供してください。

【責任範囲】マーケティング = 集客〜申込まで。申込後の営業プロセスは対象外。
- 申込数のトレンド（前月比・半年平均比）
- チャネル別の申込数増減を重点的に分析（どのチャネルが伸びている/下がっている）
- チャネル別CPA効率・申込あたりLTVの変化
- LTV分析（見込LTV・申込あたりLTV）もマーケティング側で分析

【重要】
- チャネルごとに具体的に「○○チャネルは申込+XX件（前月比+YY%）」のように言及してください
- 前月との変化が大きいチャネルを優先的に取り上げてください

フォーマット: 各示唆を「■」で始め、太字タイトル（15字以内）→ 具体的数値を含む説明（2行以内）。簡潔に。`,

  sales: `あなたは営業戦略の専門家です。以下のデータを分析し、営業プロセスに関する経営示唆を3つ、日本語で提供してください。

【責任範囲】営業 = 申込後のパイプライン（日程確定→面談実施→成約）。集客・申込獲得は対象外。
- 日程確定率（日程確定/申込）の変化
- 実施率（実施/日程確定）の変化
- 成約率（成約/営業完了）の変化
- 人材紹介の確定・見込状況
...
- ファネルの各段階の転換率の変化に注目してください
- LTVや申込数の話題はマーケティング側の管轄なので触れないでください
